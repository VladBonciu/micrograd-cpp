...
std::shared_ptr<Value> Value::operator+(std::shared_ptr<Value>& src)
{
	...
	std::shared_ptr<Value> new_value = std::make_shared<Value>(new_data, std::vector<std::weak_ptr<Value>>{shared_from_this(), src});
...
std::shared_ptr<Value> Value::operator-(std::shared_ptr<Value>& src)
{
	std::shared_ptr<Value> negative = std::make_shared<Value>(-1.0);
...
std::shared_ptr<Value> Value::tanh()
{
	...
	std::shared_ptr<Value> new_value = std::make_shared<Value>(new_data, std::vector<std::weak_ptr<Value>>{shared_from_this()});
	...
	new_value->_backward = [self = weak_from_this(), new_value]()
		{
			if (auto self_locked = self.lock())
			{
				...
				self_locked->grad += ((1.0 - new_value->data) * new_value->data) * new_value.get()->grad;
			}
	...
	new_value->_backward = [self = weak_from_this(), new_value]()
		{
			if (auto self_locked = self.lock())
			{
				self_locked->grad += (double)expl(self_locked->data) * new_value.get()->grad;
				//self_locked->grad += (double)expl(self_locked->data) * new_value.get()->grad;
			}
		};
	return new_value;
...
{
	double new_data = std::fmax(this->data, 0.0);
	std::shared_ptr<Value> new_value = std::make_shared<Value>(new_data, std::vector<std::weak_ptr<Value>>{shared_from_this()});
	new_value->_backward = [self = weak_from_this(), new_value]()
		{
			if (auto self_locked = self.lock())
...
{
	double alpha = 0.01;
	double new_data = std::fmax(this->data, alpha * this->data);
	std::shared_ptr<Value> new_value = std::make_shared<Value>(new_data, std::vector<std::weak_ptr<Value>>{shared_from_this()});
...
void Value::backward()
{
	...
	for (int i = 0; i < topo.size();i++) 
	{
		...
		{
			topo[i]->_backward();// Perform backward operation on each element in topological order
